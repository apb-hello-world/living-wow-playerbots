...
// First transfer adapter: a whole native stack moves bank -> empty bag slot.
// The GUID and protected quantity stay unchanged until the exact receipt.
// Splits/merges and mail need their own identity proofs, not fake consumption.
...
inline std::string TransferClaimPredicate(const ResourceClaim& c) {
    return "c.claim_id="+SqlValue(c.id)+" AND c.task_id="+SqlValue(c.task)+" AND c.actor_guid="+std::to_string(c.actor)+
        ...
        " AND c.quantity="+std::to_string(c.quantity)+" AND c.copper=0 AND c.native_reference=0 AND c.state='held'"
...
inline ClaimedOutcome BankTransferWrite(const Task& task,uint64_t expected,const OperationResult& result,
    ...
    if (!ValidBankTransfer(before) || before.task!=task.root || before.actor!=task.actor ||
        result.kind!="bank_withdraw" || result.state!=OperationState::Verified || task.phase!=Phase::Verifying)
        throw std::invalid_argument("Verified same-root bank transfer required");
    auto after=before;++after.revision;after.location="bags";
    ClaimedOutcome out;
    ...
    out.journal.statements.front()+=" AND EXISTS (SELECT 1 FROM living_activity_operation o WHERE o.operation_id="+
        SqlValue(result.id)+" AND JSON_COMPACT(JSON_EXTRACT(o.before_state,'$.native.transfer'))="+SqlValue(BankTransferIdentity(before))+')';
    const auto accepted=out.journal.receiptQuery;
    out.journal.statements.push_back("UPDATE living_activity_claim c SET c.location='bags',c.revision="+

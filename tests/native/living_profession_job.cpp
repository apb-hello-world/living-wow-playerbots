...
int main() {
    {
        ...
        assert(MatchesActiveProfessionJob(parent,main,why));
        assert(!MatchesActiveProfessionJob(parent,rod,why));
        assert(MatchesActiveProfessionJob(preparing,rod,why));
        assert(!MatchesActiveProfessionJob(preparing,main,why));
        auto wrongRecipe=rod;wrongRecipe.recipe=7428;
        assert(!MatchesActiveProfessionJob(preparing,wrongRecipe,why));
        wrongRecipe=rod;++wrongRecipe.reagents[0].perAttempt;
        assert(!MatchesActiveProfessionJob(preparing,wrongRecipe,why));
        auto staleStep=preparing;staleStep.revision=4;
        assert(!MatchesActiveProfessionJob(staleStep,rod,why));
        auto brokenStep=preparing;brokenStep.checkpoint.data="{}";
        assert(!MatchesActiveProfessionJob(brokenStep,rod,why));
        ...
        assert(MatchesActiveProfessionJob(ready,main,why));
        assert(!MatchesActiveProfessionJob(ready,rod,why));
        wrongRecipe=main;++wrongRecipe.subjectItem;
        assert(!MatchesActiveProfessionJob(ready,wrongRecipe,why));
        bool current=true;

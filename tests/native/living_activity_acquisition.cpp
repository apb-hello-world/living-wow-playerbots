...
int main() {
    ...
    }
    {
        ...
        assert(SameLocalServiceRecovery(task,action,lease,46203,46203,60000,false));
        assert(!SameLocalServiceRecovery(task,action,lease,46203,46203,59999,false));
        assert(!SameLocalServiceRecovery(task,action,lease,46203,46203,120000,true));
        assert(!SameLocalServiceRecovery(task,action,lease,46203,46204,120000,false));
        assert(!SameLocalServiceRecovery(task,action,lease,0,0,120000,false));
        auto staleLocal=task;++staleLocal.revision;
        assert(!SameLocalServiceRecovery(staleLocal,action,lease,46203,46203,120000,false));
        staleLocal=task;staleLocal.phase=Phase::Paused;
        assert(!SameLocalServiceRecovery(staleLocal,action,lease,46203,46203,120000,false));
        staleLocal=task;staleLocal.accepted=false;
        assert(!SameLocalServiceRecovery(staleLocal,action,lease,46203,46203,120000,false));
        staleLocal=task;staleLocal.mode=Mode::Observe;
        assert(!SameLocalServiceRecovery(staleLocal,action,lease,46203,46203,120000,false));
        auto staleLocalLease=lease;++staleLocalLease.generation;
        assert(!SameLocalServiceRecovery(task,action,staleLocalLease,46203,46203,120000,false));
        staleLocalLease=lease;++staleLocalLease.context.mapGeneration;
        assert(!SameLocalServiceRecovery(task,action,staleLocalLease,46203,46203,120000,false));

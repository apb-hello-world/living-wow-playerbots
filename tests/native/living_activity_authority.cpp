...
int main() {
    ...
    ExecutionAuthority bridge;
    auto legacy=Root(A,Priority::Delivery);
    bridge.Observe(legacy.context,0);
    auto old=bridge.AcquireCompatibility(legacy,1000,5000);
    assert(old.Granted() && bridge.Read(legacy.actor).compatibility && bridge.Size()==1);
    assert(bridge.DescribeCompatibility(old.lease,"traveling","paid_mail_collection"));
    auto legacyAction=Action(legacy,old.lease);
    assert(bridge.Authorize(move,legacy.context,1001,&legacy,&legacyAction)==AuthorityCode::ReconciliationRequired);
    assert(bridge.BeginAtomic(old.lease,B,1001).code==AuthorityCode::ReconciliationRequired);
    assert(bridge.SelectStep(old.lease,nullptr)==AuthorityCode::ReconciliationRequired);
    auto same=bridge.AcquireCompatibility(legacy,1002,5000);
    assert(same.code==AuthorityCode::Renewed && SameLease(same.lease,old.lease));
    assert(bridge.Read(legacy.actor).compatibilityReason=="paid_mail_collection");
    assert(bridge.Acquire(legacy,Movement,1003,5000).code==AuthorityCode::StaleRevision);
    auto background=Root(C,Priority::Progression);
    assert(bridge.Acquire(background,Movement,1003,5000).code==AuthorityCode::PriorityDenied);
    auto humanTask=Root(B,Priority::Human);
    auto takeover=bridge.Acquire(humanTask,Movement,1004,5000);
    assert(takeover.Granted() && SameLease(takeover.displaced,old.lease));
    assert(!bridge.Read(legacy.actor).compatibility && bridge.Size()==1);
    assert(bridge.Read(legacy.actor).compatibilityReason.empty());
    assert(bridge.Release(old.lease).code==AuthorityCode::StaleLease);
    assert(!bridge.DescribeCompatibility(old.lease,"completed","stale_callback"));
    assert(bridge.AcquireCompatibility(legacy,1005,5000).code==AuthorityCode::PriorityDenied);
    assert(SameLease(bridge.Read(legacy.actor).lease,takeover.lease));
    assert(bridge.Release(takeover.lease).code==AuthorityCode::Released);
    auto resumed=bridge.AcquireCompatibility(legacy,1006,5000);
    assert(resumed.Granted() && resumed.lease.generation>takeover.lease.generation);
    assert(bridge.Release(old.lease).code==AuthorityCode::StaleLease);
    auto moved=legacy.context;++moved.mapGeneration;
    assert(bridge.Observe(moved,0).code==AuthorityCode::Released);
    assert(!bridge.Read(legacy.actor).compatibility);
}
